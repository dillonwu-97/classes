...
{	
	...
	r4 = (int)&endp5;
	DO_PASSERN(r4);					/* P(endp5)     */ // block p1, enter p4
...
p4()
{
	...
	r4 = (int)&synp4;
	DO_VERHOGEN(r4);				/* V(synp4)     */ // unlock p4 
...
p5()
{
	...
	r2 = PROGTRAP; // where is r2/d2 being stored? what value is this changing and how does one access it?
	r3 = (int)&pstat_o;
	r4 = (int)&pstat_n;
	DO_SPECTRAPVEC();
	...
	tstate.s_sr.ps_m = TRUE;	/* lation on with an invalid  */
                                        /* CPU Root Pointer                 */
// this kills all of p5 but i dont think that is supposed to happen 
	LDST(&tstate);
}
...
p5sys()
{
	...
	case 0:
		...
		break;
	case 1:
		print("high level SYS call from kernel mode process"); // the first time this is triggered, is it supposed to be in kernel mode?
		break;
	}
	LDST(&sstat_o);
}
