...
/*
  ...
  */
    ...
    public static void placementMulti (int number, Stack<Integer> input) {
    ...
    public static void placement (int number, Stack<Integer> input) {
      // tracker keeps track of which column position we are in
      int rTracker = 0; // tracks the row
      int cTracker = 0; // tracks the column position
      //backtrack counter?
      ...
      while (rTracker < number) {
        ...
        while (cTracker < number) {
          // Need something to test the next box in the row because this currently doesn't do that
    ...
    // start at input.size() because all the numbers in the stack represent the previous columns
      for (int i = input.size(); i > 0; i--) {
        // I have to put in i - 1 because I'm looking at the indices, which does not have a 0
          ...
          if (((column - check(i, input)== 1) && (row-(i-1) == 1)) ||
              ((column - check(i, input)== -1) && (row-(i-1) == 1)) ) {
            return 1;
