...
class EmergencyGenerator implements Refuelable {
    ...
    public double powerUp(double hours) {
        double maxUptime = tank * 5;
        double actualUptime;
        if (maxUptime < hours) {
            actualUptime = maxUptime;
        } else {
            actualUptime = hours;
        }
        double fuelConsumed = actualUptime / 5;
        tank -= fuelConsumed;
        System.out.println("My generator was on for " + actualUptime +
                " hours and consumed " + fuelConsumed + " gallons of fuel.");
        return actualUptime;
    }
...
class Car implements Refuelable, Driveable {
    ...
    public double drive(double miles) {
        double actualMiles = tank * milesPerGallon;
        if (actualMiles > miles) {
            actualMiles = miles;
        }
        double fuelConsumed = actualMiles / milesPerGallon;
        tank -= fuelConsumed;
        System.out.println("I drove my car for " + actualMiles + 
                " miles and consumed " + fuelConsumed + " gallons of fuel.");
        return actualMiles;
    }
    ...
    public String toString() {
        return "Car with " + tank + " out of " + tankCapacity
                + " gallons of fuel; miles per gallon: " + milesPerGallon;
    }
}
...
class Bicycle implements Driveable {
    ...
    public double drive(double miles) {
        ...
        double calories = 30 * actualMiles;
        System.out.println("I rode my bike for " + actualMiles + 
                " miles and burned " + calories + " calories.");
        return actualMiles;
    }
