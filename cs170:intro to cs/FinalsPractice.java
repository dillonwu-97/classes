...
class Marathoner extends Athlete implements Runner {
    ...
    public boolean compete(String raceName) {
        for (int i = 0; i < numRaces; i++) {
            if (raceNames[i].equals(raceName) && getType(raceNames[i]).equals("run")) {
...
class Triathlete extends Athlete implements Runner, Swimmer{
    ...
    public boolean compete(String raceName) {
        for (int i = 0; i < numRaces; i++) {
            if ((getType(raceNames[i])).equals("run") && raceNames[i].equals(raceName)) {
                System.out.println(run(raceName, raceLocations[i]));
                completedRuns++;
                System.out.println("I completed the run " + raceName);
                return true;
            } else if ((getType(raceNames[i])).equals("swim") && raceNames[i].equals(raceName)) {
