...
class PriorityQueue:
    ...
    # Initialize queue, min_index is always at index 0
    def __init__ (self):
        self.queue = []
    ...
    # The insert must maintain heap priority
    def append(self, key):
        if key is None:
            raise ValueError('Cannot insert None into the queue')
        self.queue.append(key)
        # Compare key with parent
        child = self.__len__() - 1
        parent = (child -1) // 2
        # -1 is to stop underflow error 
        while (self.queue[child] < self.queue[parent] and parent!=-1):
            self.queue [child], self.queue[parent] = self.queue[parent], self.queue[child]
            child = parent
            parent = (child -1) // 2

    # Returns min value in queue
    def min(self):
        if len(self.queue) == 0:
            return None
        return self.queue[0]
    # Performs heapificiation
    def heapify(self, index):
        left = index * 2 + 1
        right = index * 2 + 2
        mini = index # stores the index with the min
        if left < self.__len__():
            if self.queue[index] > self.queue[left]:
                mini = left
            else:
                mini = index
        if right < self.__len__():
            if self.queue[right] < self.queue[mini]:
                mini = right
        # Swap
        if mini!= index:
            self.queue[index], self.queue[mini] = self.queue[mini], self.queue[index]
            self.heapify(mini)
            
    # Extraction method and maintain heap priority
    def pop(self):
        if len(self.queue) == 0:
            return None
        val = self.min() # get the value
        last = self.__len__() -1 # swap last and first value
        self.queue[0], self.queue[ last ] = self.queue[ last ], self.queue[0]
        self.queue.pop( last ) # pop the last value
        self.heapify(0) # reheapify the heap
        return val
